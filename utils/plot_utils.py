...
def compare_experiments(
	...
	for i, experiment in enumerate(experiment_list):
		...
		if cleanPlotMode:
			axs[0].plot(
				x, np.average(avg, axis=0), color=first_col, label=experiment_names[i], lw=2, zorder=3 * (i + 1) + 2
			)
			axs[0].fill_between(
				x,
	...
	if len(experiment_list) == 2:
		auc = compute_metrics_comparison(experiment_list, "AUC")
		final_best = compute_metrics_comparison(experiment_list, "final_best")
		print("with AUC metric: p({} < {}) = {}".format(experiment_names[0],experiment_names[1], auc))
		print("with final best metric: p({} < {}) = {}".format(experiment_names[0],experiment_names[1], final_best))
...
"""
	...
	metric: "AUC" (Area under the curve),
			"final_best"
"""
def compute_metrics_comparison(experiment_list, metric="AUC"):
	assert len(experiment_list) == 2
	for i, experiment in enumerate(experiment_list):
		if isinstance(experiment, str) and isdir(experiment):
			experiment_list[i] = load_experiment(experiment)

	def compute_area_metrics(experiment_list):
		num_run = len(experiment_list[0])
		...
		for i, experiment in enumerate(experiment_list):
			for j, run in enumerate(experiment):
				...
				min_fit_run = np.min(run["fitness"], axis=1) + shift
	...
	def compute_final_best_metrics(experiment_list):
		num_run = len(experiment_list[0])
		# metrics of each run for both the experiments
		metrics = np.zeros(shape=[2, num_run])
	...
	metric = metric.lower()
	if metric == "auc":
		metrics = compute_area_metrics(experiment_list)
	elif metric == "final_best":
		metrics = compute_final_best_metrics(experiment_list)
