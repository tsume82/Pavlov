...
class DifferentialEvolutionSolver:
	...
	def solve(self):
		...
		if self._wrapped_constraints:
			...
			if DE_result.maxcv > 0:
				...
				DE_result.message = "The solution does not satisfy the" " constraints, MAXCV = " % DE_result.maxcv
	...
	def _mutate(self, candidate):
		...
		if np.shape(scale) and np.shape(scale)[0] > 1:
			scale = scale[candidate]
			cross_over_probability = self.cross_over_probability[candidate]

		if self.strategy in ["currenttobest1exp", "currenttobest1bin"]:
			bprime = self.mutation_func(candidate, self._select_samples(candidate, 5), scale)
		else:
			bprime = self.mutation_func(self._select_samples(candidate, 5), scale)
	...
	def _best1(self, samples, scale):
		"""best1bin, best1exp"""
		r0, r1 = samples[:2]
		return self.population[0] + scale * (self.population[r0] - self.population[r1])

	def _rand1(self, samples, scale):
		"""rand1bin, rand1exp"""
		r0, r1, r2 = samples[:3]
		return self.population[r0] + scale * (self.population[r1] - self.population[r2])

	def _randtobest1(self, samples, scale):
		"""randtobest1bin, randtobest1exp"""
		r0, r1, r2 = samples[:3]
		bprime = np.copy(self.population[r0])
		bprime += scale * (self.population[0] - bprime)
		bprime += scale * (self.population[r1] - self.population[r2])
		return bprime

	def _currenttobest1(self, candidate, samples, scale):
		"""currenttobest1bin, currenttobest1exp"""
		r0, r1 = samples[:2]
		bprime = self.population[candidate] + scale * (
			self.population[0] - self.population[candidate] + self.population[r0] - self.population[r1]
		)
		return bprime

	def _best2(self, samples, scale):
		"""best2bin, best2exp"""
		r0, r1, r2, r3 = samples[:4]
		bprime = self.population[0] + scale * (
			self.population[r0] + self.population[r1] - self.population[r2] - self.population[r3]
		)

		return bprime

	def _rand2(self, samples, scale):
		"""rand2bin, rand2exp"""
		r0, r1, r2, r3, r4 = samples
		bprime = self.population[r0] + scale * (
